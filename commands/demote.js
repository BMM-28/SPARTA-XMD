...
async function demoteCommand(sock, chatId, mentionedJids, message) {
    try {
        ...
        const demotionMessage = `*『 𝐆𝐑𝐎𝐔𝐏 𝐃𝐄𝐌𝐎𝐓𝐈𝐎𝐍 』*\n\n` +
            `👤 *𝑫𝒆𝒎𝒐𝒕𝒆𝒅 𝑼𝒔𝒆𝒓${userToDemote.length > 1 ? 's' : ''}:*\n` +
            `${usernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *𝑫𝒆𝒎𝒐𝒕𝒆𝒅 𝑩𝒚:* @${message.key.participant ? message.key.participant.split('@')[0] : message.key.remoteJid.split('@')[0]}\n\n` +
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `*『 𝐆𝐑𝐎𝐔𝐏 𝐃𝐄𝐌𝐎𝐓𝐈𝐎𝐍 』*\n\n` +
            `👤 *𝑫𝒆𝒎𝒐𝒕𝒆𝒅 𝑼𝒔𝒆𝒓${participants.length > 1 ? 's' : ''}:*\n` +
            `${demotedUsernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 *𝑫𝒆𝒎𝒐𝒕𝒆𝒅 𝑩𝒚:* ${demotedBy}\n\n` +
            `📅 *𝑫𝒂𝒕𝒆:* ${new Date().toLocaleString()}`;
